...
import { acceptRecruitmentById, deleteByID, getAllRecruitmentOfRecruiter } from "../util/APIUtils";
...
class Recruitment extends React.Component {
	...
	handleAcceptRecruitment = (id) => {
		const userConfirmed = window.confirm("Bạn có chắc chắn muốn nhận ứng viên này?");

		if (userConfirmed) {
			this.setState({ clicked: true });
			acceptRecruitmentById(id).then(response => {
			
			}).catch(data => {
				Alert.success("Đã chấp nhận ứng viên này!!") 
			});
		}else {
			console.log("Đã hủy bỏ đồng ý nhận.");
		}
							...
							<table class="table">
								<thead>
									<tr>
										...
										<th style={{ width: "10%" }}>Xem CV</th>
										<th style={{ width: "15%" }}>Ứng Tuyển</th>
										...
										if(adv.upload) {
											return (
												<tr>
													...
													<a  href={adv.cv === null ? "" : `http://localhost:8080/document/`+adv.cv.replace('photographer/files/','')} style={{ color : "white" }} target="_blank">Xem</a>    
													</button></td>
													<td><button onClick={() => this.handleAcceptRecruitment(adv.id)} type="button" class="btn btn-success">Đồng ý nhận</button></td>
											...
											return (
												<tr>
													...
													<td>{adv.jobseeker.user.email}</td>
													<td>{adv.jobseeker.user.name}</td>
													<td><PdfGenerator imageLink={adv.avatar} jsonString={adv.cvDetail}/></td>
													<td><button onClick={() => this.handleAcceptRecruitment(adv.id)} type="button" class="btn btn-success">Đồng ý nhận</button></td>
													<td style={{ color : "green" }}>{adv.isAnswer === true ? "Đã Liên Hệ" : "Chưa Liên Hệ"}</td>
													<td class="table-action">
														&nbsp;<a href="#" onClick={() => this.handleSendEmail(adv.id)}><svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 512 512"><path d="M64 112c-8.8 0-16 7.2-16 16v22.1L220.5 291.7c20.7 17 50.4 17 71.1 0L464 150.1V128c0-8.8-7.2-16-16-16H64zM48 212.2V384c0 8.8 7.2 16 16 16H448c8.8 0 16-7.2 16-16V212.2L322 328.8c-38.4 31.5-93.7 31.5-132 0L48 212.2zM0 128C0 92.7 28.7 64 64 64H448c35.3 0 64 28.7 64 64V384c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V128z" /></svg></a>
