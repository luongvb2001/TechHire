...
public class JobseekerServiceImpl extends BaseService implements JobseekerService {
    ...
    public String addCV(String name, MultipartFile fileCv) {
        ...
        CurriculumVitae curriculumVitae = new CurriculumVitae(name,file, false, true, getUser().getJobseeker());
        curriculumVitaeRepository.save(curriculumVitae);
        return "Thêm mới CV thành công!!";
    }

    @Override
    public String createCV(String name, MultipartFile avatarCv, String cvDetail) {
        String avatar = fileStorageService.storeFile(avatarCv);
        CurriculumVitae curriculumVitae = new CurriculumVitae(name, false, avatar, cvDetail, false, getUser().getJobseeker());
        curriculumVitaeRepository.save(curriculumVitae);
        return "Thêm mới CV thành công!!";
    }
    ...
    public void submitRecruitment(Long jobId) {
        ...
        for (int i = 0; getUser().getJobseeker().getCurriculumVitaeList().size() > i ; i++) {
            if (getUser().getJobseeker().getCurriculumVitaeList().get(i).getStatus()) {
                ...
                recruitment.setAvatar(getUser().getJobseeker().getCurriculumVitaeList().get(i).getAvatar());
                recruitment.setCvDetail(getUser().getJobseeker().getCurriculumVitaeList().get(i).getCvDetail());
                recruitment.setUpload(getUser().getJobseeker().getCurriculumVitaeList().get(i).getUpload());
            }
        }
    ...
    @Override
    public Page<RecruitmentResponse> getRecruitmentOfSeeker(String createAt, String jobName, Integer pageNo, Integer pageSize) {
        int page = pageNo == 0 ? pageNo : pageNo - 1;
        Pageable pageable = PageRequest.of(page, pageSize);
        return mapper.convertToResponsePage(
                recruitmentRepositoryCustom.getRecruitmentOfRecruiter(pageable,createAt, getUser().getJobseeker().getId(), null, jobName),
    ...
    public void sendEmailFromTemplate(ContactRequest contactRequest) throws MessagingException, IOException {
        ...
        message.setFrom(new InternetAddress("luongvb2k1@gmail.com"));
        ...
        htmlTemplate = htmlTemplate.replace("VU LUONG", contactRequest.getName());
